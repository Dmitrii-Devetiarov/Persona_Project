...
"""Fusion layer: combines Streams A, B, C1, C2 into a persona vector and generates bilingual prompts."""
...
from src.fuzzy_memory import SemanticCloud, KeywordMemory
...
class PersonaManager:
    ...
    VALID_AXES = frozenset([
        AXIS_EMOTIONALITY, AXIS_FACTUAL, AXIS_VERBOSITY, AXIS_FIGURATIVENESS,
        AXIS_DISAGREEMENT, AXIS_COMFORT, AXIS_MODEL_RESISTANCE, AXIS_COMPLEXITY
    ])
    ...
    def __init__(
        ...
        if persona_path is None:
            persona_path = str(Path(__file__).parent.parent / "data" / "persona_core.json")
        ...
        self.preferences: Dict[str, float] = {axis: 0.0 for axis in self.VALID_AXES}

        self._style_sum: Optional[Dict[str, float]] = None
        self._style_weight: float = 0.0
        self._pref_sum: Optional[Dict[str, float]] = None
        self._pref_weight: float = 0.0

        self._dialog_count: int = 0
        self._last_fusion: Optional[Dict[str, float]] = None
    ...
    def set_language(self, lang: str) -> None:
        """Switch output language at runtime. Invalidates prompt cache and reloads spaCy model."""
        if lang not in LOCALES:
    ...
    def _load(self) -> bool:
        ...
        try:
            ...
            if saved_lang != self.lang:
                logger.info(f"Switching language from {self.lang} to saved {saved_lang}")
            ...
            if cloud_data.get("cloud") is not None:
                try:
                    self.fuzzy_memory = SemanticCloud.from_dict(cloud_data, model=self._encoder, nlp=self._nlp)
                except Exception as e:
            ...
            if style_data:
                ...
                self._style_weight = min(style_data.get("cumulative_weight", 0.0), self.MAX_STYLE_WEIGHT)
            ...
            if pref_cum:
                ...
                self._pref_weight = min(pref_cum.get("weight", 0.0), self.MAX_PREF_WEIGHT)
            if "keyword_memory" in data:
                try:
                    self.keyword_memory = KeywordMemory.from_dict(data["keyword_memory"], self.fuzzy_memory)
                except Exception as e:
            ...
            if not self._validate_state():
                ...
                self.reset()
                return False
            logger.info(f"Loaded persona from {self.persona_path} (v{version}, lang={self.lang})")
            return True
    ...
    def update_after_dialog(self, dialog: List[Dict[str, Any]]) -> bool:
        try:
            if not dialog:
                return True
            ...
            for i in range(len(dialog) - 1):
                if dialog[i].get("role") == "user" and dialog[i + 1].get("role") == "assistant":
                    self.keyword_memory.add_pair(dialog[i]["content"], dialog[i + 1]["content"])
            b_features = self.style_tracker.extract(user_messages)
            if self._style_sum is None:
                self._style_sum = {k: 0.0 for k in b_features}
            ...
            current_style = {k: v / self._style_weight for k, v in self._style_sum.items()} if self._style_weight > 0 else None
            c1_vector = {axis: 0.0 for axis in self.VALID_AXES}
            for msg in user_messages:
            ...
            for i in range(len(dialog) - 1):
                if dialog[i].get("role") == "assistant" and dialog[i + 1].get("role") == "user":
            ...
            c2_vector = self.implicit_tracker.get_inferred_vector() or {axis: 0.0 for axis in self.VALID_AXES}
            ...
            for axis in self.VALID_AXES:
                ...
                if abs(c2_value) > self.C2_BOOST_THRESHOLD and self._dialog_count >= self.MIN_DIALOGS_FOR_BOOST:
                    w_c2 = min(self.C2_BOOSTED_WEIGHT, w_c2 + 0.1 * (self._dialog_count - self.MIN_DIALOGS_FOR_BOOST))
                    w_b = max(0.0, self.FUSION_WEIGHTS["surface"] - (w_c2 - self.FUSION_WEIGHTS["implicit"]))
                dialog_signal[axis] = (
            ...
            if self._pref_sum is None:
                self._pref_sum = {axis: 0.0 for axis in self.VALID_AXES}
            for axis in self.VALID_AXES:
                self._pref_sum[axis] = self._pref_sum.get(axis, 0.0) + dialog_signal[axis]
    ...
    def _scale_surface_to_preferences(self, surface_ema: Optional[Dict[str, float]]) -> Dict[str, float]:
        if surface_ema is None:
            return {axis: 0.0 for axis in self.VALID_AXES}
        ...
        scaled = {
            self.AXIS_EMOTIONALITY: float(np.clip(
                emphasis * 80 + evaluative * 60 + first_person * 10
                - hedging * 25 + declarative * 0.5 - 0.3,
                -1.0, 1.0,
            )),
            self.AXIS_FACTUAL: float(np.clip(
                (1.0 - question_ratio) * 1.5 - hedging * 40 + 0.1,
                -1.0, 1.0,
            )),
            self.AXIS_VERBOSITY: 0.0,
            self.AXIS_FIGURATIVENESS: float(np.clip(
                fig_ratio / 0.05 + adj_density / 60 + avg_sent_len / 12 - 0.5,
                -1.0, 1.0,
            )),
            self.AXIS_DISAGREEMENT: 0.0,
            self.AXIS_COMFORT: float(np.clip(
                hedging * 15 + avg_sent_len * 0.03 - imperative * 20 - first_person * 15 - 0.05,
                -1.0, 1.0,
            )),
            self.AXIS_MODEL_RESISTANCE: float(np.clip(
                emphasis * 40 + declarative * 1.5 - hedging * 25 + 0.15,
                0.0, 1.0,
            )),
            self.AXIS_COMPLEXITY: float(np.clip(
                (fog_index - 8.0) / 4.0,
                -1.0, 1.0,
            )),
        }
    ...
    def _get_prompt_key(self, axis: str, value: float) -> str:
    ...
    def _get_label(self, axis: str, value: float, lang: Optional[str] = None) -> str:
    ...
    def _cached_prompt(self, preferences_tuple: Tuple[float, ...], question_hash: int, lang: str) -> str:
        """Generate full persona prompt from cached preferences. Lang is part of cache key."""
        L = LOCALES[lang]
        parts = []
    ...
    def get_persona_prompt(self, current_question: Optional[str] = None, lang: Optional[str] = None) -> str:
        """Generate persona prompt in specified language (or current if None)."""
        try:
            if lang and lang != self.lang:
                self.set_language(lang)

            preferences_tuple = tuple(self.preferences[axis] for axis in self.AXIS_ORDER)
            question_hash = hash(current_question) if current_question else 0
            base_prompt = self._cached_prompt(preferences_tuple, question_hash, self.lang)
            ...
            if current_question:
                try:
                    ...
                    if memory_keywords:
                        prefix = self.L.get("prompt_memory_prefix", "[Memory] Previously discussed:")
                        suffix = self.L.get("prompt_memory_suffix", "Use as context. [/Memory]")
                        context = f"{prefix} {', '.join(memory_keywords[:10])}. {suffix}"
            ...
            return f"{fallback}\n- {self.L.get('emotionality_label_0', 'neutral')}\n{close}"
    ...
    def __repr__(self) -> str:
        return f"PersonaManager(dialogs={self._dialog_count}, errors={self._error_count}, lang={self.lang}, persist={self._persist})"